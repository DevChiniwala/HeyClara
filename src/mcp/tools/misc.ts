...
export async function placeCall(args: {
  ...
  if (!phone) {
    return "Phone channel is not configured. Add channels.twilio.{sid, secret, public_base_url} and channels.phone.{from_number, openai_api_key} to ~/.heyclara/config.yaml (or set the matching env vars in .env), then restart the daemon.";
  }
  try {
