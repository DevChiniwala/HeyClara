...
class PhoneChannel implements Channel {
  ...
  private async handleIncoming(_req: Request, params: Record<string, string>): Promise<Response> {
    ...
    if (!this.canStartRealtime()) {
      ...
      return twimlResponse(sayAndHangupTwiML("Hi, this is Clara. The voice loop is offline right now. Try again soon."));
    }
    ...
    try {
      ...
      for (const turn of result.transcript) {
        await Message.save({
          sessionId,
          room,
          sender: turn.role === "user" ? context.remoteLabel : "clara",
          content: turn.text,
