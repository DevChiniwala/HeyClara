...
class SlackChannel implements Channel {
  ...
  /** Timestamps of messages Clara posted proactively (used to detect replies to our own messages) */
  ...
  async start(): Promise<void> {
    ...
    // Slash command: /clara
    app.command("/clara", async ({ command, ack, respond }) => {
      await ack();
      ...
      if (subcommand === "new" || subcommand === "start") {
        ...
        await respond("Commands:\n• `/clara new` — start a new conversation\n• `/clara help` — show this help");
      } else {
    ...
    // Slash command: /clara-new — quick shortcut to start a fresh conversation
    app.command("/clara-new", async ({ command, ack, respond }) => {
      await ack();
      const isDm = command.channel_name === "directmessage";
      ...
      log.info(
        ...
        "new slack session via /clara-new",
      );
      await respond("New conversation started.");
    });
    ...
    app.message(async ({ message, say, client }) => {
      ...
      // In threads where Clara already has a session (in-memory or DB), listen without @mention.
      // Also catches replies to messages Clara posted proactively (outbound tracking + bot-authored fallback).
      ...
      // When replying in a thread, fetch thread context so Clara can see the full conversation
      if (msg.thread_ts) {
        try {
