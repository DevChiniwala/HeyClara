...
import type { SaveMessageParams, RoomStats, RecentMessage, SearchResult, SessionMessage } from "../../types/message";
...
export async function getRecent(limit = 20, room?: string): Promise<RecentMessage[]> {
  ...
  const rows = room
    ? await sql`
        SELECT room, sender, content, created_at FROM messages
        WHERE room = ${room} ORDER BY created_at DESC LIMIT ${limit}
      `
    : await sql`
        SELECT room, sender, content, created_at FROM messages
...
export async function search(query: string, limit = 20, room?: string): Promise<SearchResult[]> {
  ...
  const rows = room
    ? await sql`
        SELECT session_id, room, sender, content, created_at FROM messages
        WHERE content ILIKE ${pattern} AND room = ${room}
        ORDER BY created_at DESC LIMIT ${limit}
      `
    : await sql`
        SELECT session_id, room, sender, content, created_at FROM messages
...
export async function getBySession(sessionId: string): Promise<SessionMessage[]> {
  const sql = getSql();
  const rows = await sql`
    SELECT room, sender, content, is_from_agent, created_at FROM messages
...
export async function getRecentNotifications(roomPrefix: string, limit = 10): Promise<Array<{ content: string; source: string | null; createdAt: string }>> {
  const sql = getSql();
  const pattern = `^${escapeRegex(roomPrefix)}-\\d+$`;
  const rows = await sql`
    SELECT m.content, m.metadata->>'source' AS source, m.created_at
    FROM messages m JOIN sessions s ON m.session_id = s.id
    WHERE s.room ~ ${pattern} AND m.is_from_agent = true
      AND m.metadata->>'kind' = 'notification' AND m.delivery_status != 'failed'
      AND m.created_at >= GREATEST(
        COALESCE((SELECT MAX(m2.created_at) FROM messages m2 JOIN sessions s2 ON m2.session_id = s2.id
          WHERE s2.room ~ ${pattern} AND m2.is_from_agent = false), NOW() - INTERVAL '72 hours'), NOW() - INTERVAL '72 hours')
...
export async function getRoomStats(): Promise<RoomStats[]> {
  const sql = getSql();
  const rows = await sql`
    SELECT s.room, COUNT(DISTINCT s.id)::int AS sessions, COUNT(m.id)::int AS messages, MAX(m.created_at) AS last_activity
    FROM sessions s LEFT JOIN messages m ON m.session_id = s.id
    GROUP BY s.room ORDER BY MAX(m.created_at) DESC NULLS LAST
  `;
  return rows.map((r) => ({
    room: r.room,
    sessions: r.sessions,
