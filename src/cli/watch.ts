...
const HELP = `Usage: clara watch <command>
...
export function watchCommand(): void {
  ...
  switch (sub) {
    ...
    case "add": {
      ...
      if (!name) {
        fail("Usage: clara watch add <channel_id#name> [behavior]");
      }
    ...
    case "remove": {
      const name = process.argv[4];
      if (!name) fail("Usage: clara watch remove <channel_id#name>");
    ...
    case "enable": {
      const name = process.argv[4];
      if (!name) fail("Usage: clara watch enable <channel_id#name>");
    ...
    case "disable": {
      const name = process.argv[4];
      if (!name) fail("Usage: clara watch disable <channel_id#name>");
