/**
 * `clara phone <subcommand>` — small CLI surface for the phone channel.
 *
 * Subcommands:
...
async function phoneCallCommand(): Promise<void> {
  ...
  if (!number || goalParts.length === 0) {
    fail('Usage: clara phone call <e164-number> "<goal sentence...>"');
  }
...
function helpText(): string {
  return [
    ...
    "",
    "Config lives in ~/.heyclara/config.yaml:",
    ...
    "if you prefer .env. See the clara-phone skill for full deploy walkthrough.",
  ].join("\n");
}
