...
const HELP = `Usage: clara agent <command>
...
export async function agentCommand(): Promise<void> {
  ...
  switch (subcommand) {
    case "list": {
      ...
      if (agents.length === 0) {
        console.log("No agents found. Create agents in ~/.heyclara/agents/<name>/AGENT.md");
      } else {
    ...
    case "show": {
      ...
      if (!name) fail("Usage: clara agent show <name>");
      const agents = scanAgents();
