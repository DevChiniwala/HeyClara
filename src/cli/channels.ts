...
export async function sendCommand(): Promise<void> {
  ...
  if (!message) fail("Usage: clara send [-c channel] [--to <slack-channel-id>] [--thread <ts>] <message>");
...
export function telegramCommand(): void {
  ...
  if (sub === "setup") {
    ...
    if (!token) {
      fail("Usage: clara telegram setup --bot-token=<token> [--chat-id=<id>]");
    }
    ...
    console.log("Run `clara restart` to activate.");
    return;
  }
  ...
  console.log("\nUsage: clara telegram setup --bot-token=<token> [--chat-id=<id>]");
}
...
export async function slackCommand(): Promise<void> {
  ...
  if (sub === "setup") {
    ...
    if (!botToken || !appToken) {
      fail("Usage: clara slack setup --bot-token=xoxb-... --app-token=xapp-... [--dm-user-id=U...]");
    }
  ...
  console.log("\nUsage: clara slack setup --bot-token=xoxb-... --app-token=xapp-... [--dm-user-id=U...]");
