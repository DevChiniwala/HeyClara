# Harness-Agnostic Clara — Implementation Plan
...
**Architecture:** The orchestrator (`engine.ts`/`runner.ts`) consumes only a normalized `AgentEvent` stream and never branches on backend. Each backend is a closed adapter. Clara's 21 tools become one `CLARA_TOOLS` table served two ways: in-process to Claude (today's path), and over a loopback HTTP MCP endpoint to the CLI backends (per-run frozen context). Spec: `docs/superpowers/specs/2026-06-24-harness-agnostic-backend-design.md`.
...
- **DRY:** exactly one tool table (`CLARA_TOOLS`); both transports consume it. One shared subprocess-spawn/env-scrub helper for Codex+Gemini. One JSONL-line reader shared by the CLI adapters.
...
1. **Characterization at the seam, not bolted onto the old engine (revised Task 1.0).** The current `engine.ts` imports `query()` directly and is DB-coupled; the repo has NO `mock.module` pattern (it uses a real test DB + a skip-gated live-`query` integration test). Bolting module-mocking onto soon-to-be-replaced code would be patchwork. Instead pin the hard behaviors where they are naturally testable: (a) **one-session-event-per-retry** and **abort interrupts** in `ClaudeSession` tests via an injectable `queryFn` (Task 1.4 — no DB, no module mocks); (b) **save-once / no-double-save** in an engine test at Task 1.5 using a **fake `AgentBackend`** (test-swappable registry) against the real test DB (`tests/db/setup.ts`). The existing skip-gated `engine.integration.test.ts` (live Claude) stays the end-to-end check.
...
### Task 1.1: `CLARA_TOOLS` — one tool table (DRY)
...
- Produces: `interface NiaTool { name: string; description: string; schema: z.ZodRawShape; handler: (args: any, ctx?: McpSourceContext) => Promise<string> | string }` and `export const CLARA_TOOLS: NiaTool[]`. Later tasks (the in-process server and the loopback endpoint) both map over `CLARA_TOOLS`.
...
import { CLARA_TOOLS } from "../../src/mcp/tools/table";

describe("CLARA_TOOLS", () => {
  test("contains all 21 tools with unique names", () => {
    const names = CLARA_TOOLS.map((t) => t.name);
  ...
  },
  {
    ...
    schema: {
      ...
      prompt: z
        .string()
        .describe(
          "What the job should do. A non-empty ~/.heyclara/jobs/<job-name>/prompt.md overrides this database prompt at runtime.",
        ),
...
- [ ] **Step 4: Rewrite `src/mcp/server.ts` to map over `CLARA_TOOLS`**
...
git commit -m "refactor(mcp): extract CLARA_TOOLS table; server maps over it (DRY)"
```
...
**Phase 2 done:** a Codex job runs end-to-end with full Clara tools via the endpoint; concurrent routing verified.
...
- **4.1** side-effect-tool gate (`place_call`, `send_message`) at the `CLARA_TOOLS` handler boundary (one gate, all backends); each CLI's own approval flag governs its built-ins.
...
**Type consistency:** `AgentEvent`/`AgentUsage`/`AgentSession`/`AgentBackend`/`AgentSessionContext` defined once in 1.2 and imported unchanged everywhere. `NiaTool`/`CLARA_TOOLS` (1.1) consumed by both transports. `QueryFn` local to 1.4. Each backend's normalizer (`SdkNormalizer`, `CodexNormalizer`, `GeminiNormalizer`) shares the `consume(msg): AgentEvent[]` shape.
