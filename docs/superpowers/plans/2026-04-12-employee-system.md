...
**Goal:** Add a first-class "employee" entity to heyclara — persistent, goal-driven autonomous principals that can be onboarded to projects and operate via `clara chat --employee <name>`.

**Architecture:** Employees are file-based entities in `~/.heyclara/employees/<name>/` with YAML-frontmatter EMPLOYEE.md files. A scanner discovers them (mirroring the agent scanner pattern). The chat engine and REPL are extended to accept `--employee` which swaps the system prompt to the employee's identity + state. CLI commands manage the lifecycle.

**Tech Stack:** TypeScript, Bun, js-yaml, existing heyclara patterns (scanner, CLI, chat engine)
...
const HELP = `Usage: clara employee <command>
  ...
  switch (subcommand) {
    case "list": {
      ...
      if (employees.length === 0) {
        console.log("No employees. Create one with: clara employee add <name> --project <project> --repo <path>");
      } else {
    ...
    case "show": {
      ...
      if (!name) fail("Usage: clara employee show <name>");
    ...
    case "pause": {
      const name = process.argv[4];
      if (!name) fail("Usage: clara employee pause <name>");
    ...
    case "resume": {
      const name = process.argv[4];
      if (!name) fail("Usage: clara employee resume <name>");
    ...
    case "remove": {
      const name = process.argv[4];
      if (!name) fail("Usage: clara employee remove <name>");
      const emp = getEmployee(name);
      if (!emp) fail(`Employee "${name}" not found.`);
      const dir = getEmployeeDir(name);
...
Expected: "No employees. Create one with: clara employee add <name> --project <project> --repo <path>"
...
export async function employeeAdd(): Promise<void> {
  ...
  const name = args[0];
  if (!name || name.startsWith("--")) fail("Usage: clara employee add <name> --project <label> --repo <path>");
  ...
  console.log(`\nStart onboarding with: clara chat --employee ${name}`);
}
```
...
Expected: Creates `~/.heyclara/employees/testjames/` with all files
