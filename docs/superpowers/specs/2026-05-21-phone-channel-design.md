# Phone Channel — Voice Calling for Clara
...
Add real-time voice calling to Clara so the owner can:

- **Call Clara** from a phone and talk through tasks, memory, and jobs with full context.
- **Receive calls from Clara** for scheduled rituals (morning standup, evening retro), approval pings, and substantive reminders.
- **Have Clara call others** (allowlisted contacts, vendors, appointments) via an MCP tool.
...
No Twilio SDK dependency — minimal `fetch` + `crypto.createHmac` for signature validation. Keeps Clara's surface area small.

## Architecture

```
+-----------+    PSTN     +----------+   HTTP(s)   +------------------+   wss   +----------------+
|  Caller   |  <------>   |  Twilio  |  <------->  |  cloudflared     |  <-->   |  clara daemon    |
+-----------+             +----------+             |  tunnel          |         |  (Bun.serve)   |
                              |                    +------------------+         |  HTTP + WS     |
                              |  Media Streams                                  +----------------+
                              |  (wss, g711_ulaw)                                    |
...
place_call({
  ...
  goal: string,            // What Clara should accomplish — becomes realtime instructions
...
3. Allowlisted: personalized greeting (configurable per number in `~/.heyclara/phone/contacts.yaml`), reduced tool surface (no `create_job`, no `update_beads_task`).
...
Phone config lives in `~/.heyclara/config.yaml` under `channels.phone`
...
channels:
  phone:
    ...
    from_number: "+1..." # Twilio number Clara dials from
    owner_number: "+91..." # Owner — highest-trust caller
    public_base_url: https://clara.example.com
