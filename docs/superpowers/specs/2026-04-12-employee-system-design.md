...
Employees are persistent, goal-driven entities that live inside Clara. Unlike agents (role prompts for delegation) or skills (knowledge injection), an employee is an autonomous principal — it has identity, memory, goals, and the authority to create work (jobs, sub-employees, agents) within its scoped project.
...
`clara employee add james --project aicodeusage.com --repo /path/to/repo`

- Scaffolds `~/.heyclara/employees/james/` with EMPLOYEE.md (status: onboarding) and empty state files
...
Via `clara chat --employee james`. Each session:
...
```bash
clara employee pause james     # status → paused
clara employee resume james    # status → active
```

## CLI Commands

```
clara employee add <name>           Create employee and start onboarding
...
clara employee list                 List all employees with status
clara employee show <name>          Show employee details and state
...
clara employee approvals [name]     Show pending approvals

clara chat --employee <name>        Open chat session as employee
```

## System Prompt Integration

When `clara chat --employee james` runs:
...
The employee does NOT use Clara's identity files (identity.md, soul.md, etc.). The employee IS its own identity.

## Employee Authority & Tools

Employees can use all standard Clara tools plus:
...
User reviews via `clara employee approvals james` or in chat. When approved, employee updates the entry to `[approved]` with date.
