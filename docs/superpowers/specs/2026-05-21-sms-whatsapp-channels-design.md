# SMS + WhatsApp Channels for Clara
...
Add Twilio SMS (inbound/outbound on existing `+1 302 548 0697`) and Twilio WhatsApp Sandbox (inbound/outbound) as two new channels under `src/channels/`. Together with the existing phone, telegram, and slack channels, this rounds out Clara's reachability across:
...
**Backward compat (one release cycle):** if the new keys are absent, fall back to reading the old `channels.phone.{twilio_sid, twilio_secret, twilio_auth_token, owner_number, allowlist, public_base_url, port}`. This keeps the Mac Mini's existing `~/.heyclara/config.yaml` working without manual migration.
