# Harness-Agnostic Clara — MCP-Hybrid, Failover-First
...
**Round-trip CONFIRMED end-to-end (real codex 0.142.0):** a loopback Streamable-HTTP MCP endpoint on `@modelcontextprotocol/sdk@1.27.1`'s `WebStandardStreamableHTTPServerTransport` + `Bun.serve` works. `codex exec -c mcp_servers.clara.url=… -c mcp_servers.clara.bearer_token_env_var=…` connected, the endpoint validated the `Authorization: Bearer` header, the tool handler ran **in-process**, and codex returned the result. The Phase 2 mechanism is verified, not assumed.
...
2. **MCP wiring is an adapter responsibility.** The orchestrator hands an adapter a frozen `McpSourceContext` + the relay's `{url, token}`; the adapter serializes it into its own dialect (Claude: in-process SDK server; Codex: TOML `[mcp_servers.clara]`; Gemini: `settings.json` `mcpServers.clara`).
...
- **A rich approval UI.** The policy gate is minimal: Clara's side-effect tools gated at the tool boundary; each CLI's own approval flag governs its built-ins.
...
export interface AgentSessionContext {
  ...
  cwd: string;
  model?: string;
  source: McpSourceContext; // frozen routing identity for Clara's tools
...
Extract the 21 tool definitions (name, description, zod schema, handler) out of `src/mcp/server.ts` into a single exported `CLARA_TOOLS` array. Both transports consume it:

- `createClaraMcpServer(ctx)` (existing in-process SDK server, for Claude) maps over `CLARA_TOOLS` — no behavior change.
- The loopback relay (below) maps over the **same** `CLARA_TOOLS`.
...
A `127.0.0.1` HTTP server (ephemeral port) using `@modelcontextprotocol/sdk`'s `WebStandardStreamableHTTPServerTransport` + low-level `Server`, dispatching into `CLARA_TOOLS`. Started in `daemon.ts` after `setMcpFactory`; stopped in shutdown.
...
- **Failover** is a thin wrapper the consumer uses: run the primary; on a terminal `error` event with `providerDown: true` (after the backend's own retries), re-open the session on the next backend in the list with the same context, replaying conversation history from Clara's DB. No cross-backend session resume is attempted (different id spaces); continuity comes from Clara's own `messages` transcript.
...
- **Clara's side-effect tools** (`place_call`, `send_message`) gated once, at the `CLARA_TOOLS` handler boundary (covers all backends and both transports).
- **Each CLI's own built-ins** (Codex/Gemini bash/edit) governed by that CLI's approval flag, not double-gated through Clara.

## Migration (each phase ships independently testable software)

- **Phase 1 — seam + Claude extraction (behavior-preserving).** `AgentBackend`/`AgentEvent`, extract `CLARA_TOOLS`, `ClaudeBackend` (wraps today's query()+warm stream+retry+normalization), route `engine.ts`/`runner.ts` through it with Claude as the only backend. Zero behavior change. De-risks the riskiest rewrite first.
...
- **Failover continuity — LOW.** A failover starts a fresh session on the fallback backend, rebuilding context from Clara's DB transcript (the dead primary session can't resume). Acceptable; it's an outage path.

## Success criteria (v1)

1. Claude chat + jobs behave identically to today (the `ClaudeBackend` extraction is invisible).
2. A Codex job and a Gemini job each run to completion with full Clara tools (incl. `send_message` to deliver results).
