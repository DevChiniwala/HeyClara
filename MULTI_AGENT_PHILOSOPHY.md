...
Clara is a single-agent system by design. We don't have a "marketing agent" and a "coding agent" and a "research agent" — we have Clara, with skills, jobs, and MCP tools. This is not a limitation. It's a deliberate architectural choice backed by research, practitioner consensus, and our own experience.
...
HeyClara has three levels of specialization, each solving a different problem:
...
Same context        Same Clara            Fresh context
```

### Skills — Knowledge injection

Skills are specialized instructions loaded into Clara's context. They teach Clara *how* to do something without changing who Clara is.

```
~/.heyclara/skills/pr-reviewer/SKILL.md
~/.heyclara/skills/image-generation/SKILL.md
```
...
| Context | Mechanism |
|---------|-----------|
| **Chat** | Agent definitions passed to SDK's `agents` param in `query()`. SDK decides when to delegate based on `description`. Agent runs in fresh context, returns result to Clara. |
| **Jobs** | `clara job add blog "0 9 * * *" "Write a post" --agent marketer` — agent body becomes the job's system prompt. Agent model respected. |
| **MCP** | `list_agents` tool lets Clara see available agents at runtime. |
...
**Discovery:** Scanned from `agents/` directories (project, `~/.heyclara/agents/`, `~/.shared/agents/`). Same pattern as skills scanner. First match wins for deduplication.
...
- **Clara decides** — the main agent chooses when to delegate based on the description

## How It All Connects

```
User says "write a Twitter thread" on Telegram
  → Clara sees marketer agent description matches
  → SDK spawns marketer as subagent (fresh context)
  → Marketer drafts the thread
  → Result returns to Clara → sent to user
...
2. **Can an agent handle this?** Agent files give Clara a domain specialist to delegate to — via chat or scheduled jobs.
...
```bash
clara agent list              # List all available agents
clara agent show marketer     # Show agent details and prompt
clara job add blog "0 9 * * *" "Write a post" --agent marketer  # Job with agent
```
