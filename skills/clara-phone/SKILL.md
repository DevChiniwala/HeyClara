...
name: clara-phone
description: >
  Use when setting up, deploying, or debugging Clara's Twilio-based channels:
  ...
  webhook wiring, `clara phone` CLI, `place_call` / `send_message` MCP
  ...
  "realtime", "media stream", "cloudflared", or when deploying Clara to
...
- **Phone** (`src/channels/phone/`) — voice via Twilio Programmable
  ...
  (`place_call` MCP tool / `clara phone call` CLI).
...
channels:
  twilio:
    ...
    public_base_url: https://clara.example.com # No trailing slash
...
`clara phone status` prints which fields are set across all three channels.
...
```bash
sudo cloudflared --config ~/.heyclara/cloudflared-config.yaml service install
```
...
Then set in `.env`: `PUBLIC_BASE_URL=https://clara.example.com`.

Verify with `curl https://clara.example.com/healthz` — should return `ok`
...
curl -X POST "https://api.twilio.com/2010-04-01/Accounts/<AC...>/IncomingPhoneNumbers/<PN...>.json" \
  -u "<SID>:<SECRET>" \
  --data-urlencode "VoiceUrl=https://clara.example.com/twilio/voice/incoming" \
  --data-urlencode "SmsUrl=https://clara.example.com/twilio/sms/incoming" \
  --data-urlencode "StatusCallback=https://clara.example.com/twilio/voice/status"
```
...
```bash
clara phone status                                   # show config + missing fields
clara phone call <+E164> "<goal sentence>"           # one-shot outbound smoke test
```
...
place_call({
  ...
  goal: string,          // What Clara should accomplish — seeded into session instructions
...
```bash
clara job add morning-standup "0 8 * * *" \
...
5. **Send the opener immediately on session open, don't wait for first
   ...
   first, which defeats "Clara calling you".
