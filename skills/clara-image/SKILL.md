---
name: clara-image
description: This skill should be used when the user asks to "generate a Clara image", "create a profile picture for Clara", "update Clara's avatar", "make a new Clara photo", or needs to generate images of Clara with consistent identity using Gemini image generation.
---

# Clara Image Generation

Generate photorealistic images of Clara with consistent identity across different scenes, poses, and contexts using Gemini's image generation API.
...
1. `~/.heyclara/images/reference.webp` — user's custom reference (takes priority)
2. `assets/clara-reference.webp` — default shipped with heyclara

| Location | Purpose |
|----------|---------|
| `~/.heyclara/images/reference.webp` | User's reference image |
| `~/.heyclara/images/profile.webp` | User's profile picture (for Telegram/Slack) |
| `~/.heyclara/images/` | Output directory for new generations |
| `assets/clara-reference.webp` | Default reference (fallback) |
| `assets/clara-profile.webp` | Default profile picture (fallback) |
...
```bash
# With reference (default — uses assets/clara-reference.webp)
...
|------|---------|-------------|
| `--prompt` | Default Clara portrait | Image generation prompt |
| `--reference` | `assets/clara-reference.webp` | Reference image for identity |
...
| `--output` | `~/.heyclara/images/` | Output path or directory |
