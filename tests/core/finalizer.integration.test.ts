...
afterEach(async () => {
  ...
  if (process.env.CLARA_HOME) writeFileSync(`${process.env.CLARA_HOME}/config.yaml`, "");
  resetConfig();
});
...
describe("finalizer: concurrent enqueue dedupe", () => {
  ...
  test("finalizeSession does not enqueue when session finalization is disabled", async () => {
    ...
    getConfig();
    writeFileSync(`${process.env.CLARA_HOME}/config.yaml`, "session_finalization:\n  enabled: false\n");
  ...
  test("processPending marks pending requests done when no finalization tasks are enabled", async () => {
    ...
    writeFileSync(
      `${process.env.CLARA_HOME}/config.yaml`,
